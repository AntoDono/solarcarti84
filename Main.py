...
def K():C='. ';B=1;A(D(B)+C+'Calculate solar array area');B+=1;A(D(B)+C+'Calculate Array Voltage');B+=1;A(D(B)+C+'Calculate Amp Hour');B+=1;A(D(B)+C+'Calculate Parallel Batteries');B+=1;A(D(B)+C+'Calculate Power');B+=1;A(D(B)+C+'Supercharged Voltage');B+=1;A(D(B)+C+'Returns velocity');B+=1;A(D(B)+C+'Brute force best combination for series and parallel')
def L():
...
def I():
	g='Please input the number of cells: ';C();A('Staten Island SolarCar\n');K();F=E(B('\nSelect an option > '))
	...
	elif F==5:import getPow as W;C();X=E(B('Voltage of the whole array: '));Y=E(B('Please input the current: '));D=W.getPow(X,Y);A('The power is: %f'%D);B(G);C()
	...
	elif F==8:import brute;C();d=E(B('Please provide the nominal (desired) voltage: '));F=L();e=N();f=M();D=brute.bruteForce(d,F,priority=e,verbose=f);A('Best result found:');A(D);A('===================RESULT=====================');A('Batteries in parallel | %f'%D[0]);A('Batteries in series | %f'%D[1]);A('Total Voltage | %f'%D[-1]);A('Amp Hours | %f'%D[-2]);A('Total power | %f'%D[2]);A('==============================================');B(G)
	I()
I()